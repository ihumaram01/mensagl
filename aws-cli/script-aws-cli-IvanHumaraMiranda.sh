...
KEY_NAME="ssh-mensagl-2025-IvanHumara"
VPC_NAME="VPC-mensagl-2025-IvanHumara"
...
VPC_ID=$(aws ec2 create-vpc --cidr-block 10.212.0.0/16 --query 'Vpc.VpcId' --output text)
aws ec2 modify-vpc-attribute --vpc-id $VPC_ID --enable-dns-support
aws ec2 modify-vpc-attribute --vpc-id $VPC_ID --enable-dns-hostnames

# Asignar nombre a la VPC
aws ec2 create-tags --resources $VPC_ID --tags Key=Name,Value="VPC-Mensagl-2025-IvanHumara"

# Subredes en 2 AZs
SUBNET_PUBLIC1_ID=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block 10.212.1.0/24 --availability-zone ${REGION}a --query 'Subnet.SubnetId' --output text)
SUBNET_PUBLIC2_ID=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block 10.212.2.0/24 --availability-zone ${REGION}b --query 'Subnet.SubnetId' --output text)

SUBNET_PRIVATE1_ID=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block 10.212.3.0/24 --availability-zone ${REGION}a --query 'Subnet.SubnetId' --output text)
SUBNET_PRIVATE2_ID=$(aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block 10.212.4.0/24 --availability-zone ${REGION}b --query 'Subnet.SubnetId' --output text)
...
INSTANCE_PROXY1_ID=$(aws ec2 run-instances --image-id $AMI_ID --instance-type t2.micro --key-name $KEY_NAME --subnet-id $SUBNET_PUBLIC1_ID --private-ip-address 10.212.1.10 --security-group-ids $SG_PROXY_ID --associate-public-ip-address --query 'Instances[0].InstanceId' --output text)
aws ec2 create-tags --resources $INSTANCE_PROXY1_ID --tags Key=Name,Value="Proxyinverso1"

INSTANCE_PROXY2_ID=$(aws ec2 run-instances --image-id $AMI_ID --instance-type t2.micro --key-name $KEY_NAME --subnet-id $SUBNET_PUBLIC2_ID --private-ip-address 10.212.2.10 --security-group-ids $SG_PROXY_ID --associate-public-ip-address --query 'Instances[0].InstanceId' --output text)
aws ec2 create-tags --resources $INSTANCE_PROXY2_ID --tags Key=Name,Value="Proxyinverso2"

# XMPP Servers (Mensajería 1 y 2)
INSTANCE_XMPP1_ID=$(aws ec2 run-instances --image-id $AMI_ID --instance-type t2.micro --key-name $KEY_NAME --subnet-id $SUBNET_PRIVATE1_ID --private-ip-address 10.212.3.10 --security-group-ids $SG_XMPP_ID --query 'Instances[0].InstanceId' --output text)
aws ec2 create-tags --resources $INSTANCE_XMPP1_ID --tags Key=Name,Value="Mensajeria1"

INSTANCE_XMPP2_ID=$(aws ec2 run-instances --image-id $AMI_ID --instance-type t2.micro --key-name $KEY_NAME --subnet-id $SUBNET_PRIVATE1_ID --private-ip-address 10.212.3.20 --security-group-ids $SG_XMPP_ID --query 'Instances[0].InstanceId' --output text)
...
INSTANCE_SOPORTE1_ID=$(aws ec2 run-instances --image-id $AMI_ID --instance-type t2.micro --key-name $KEY_NAME --subnet-id $SUBNET_PRIVATE2_ID --private-ip-address 10.212.4.10 --security-group-ids $SG_CMS_ID --query 'Instances[0].InstanceId' --output text)
aws ec2 create-tags --resources $INSTANCE_SOPORTE1_ID --tags Key=Name,Value="Soporte1"

INSTANCE_SOPORTE2_ID=$(aws ec2 run-instances --image-id $AMI_ID --instance-type t2.micro --key-name $KEY_NAME --subnet-id $SUBNET_PRIVATE2_ID --private-ip-address 10.212.4.20 --security-group-ids $SG_CMS_ID --query 'Instances[0].InstanceId' --output text)
...
echo "-----------------------------------------"
echo "✅ La infraestructura de AWS a sido creada con exito ✅"